...
class PredictorGateway(mlflow.pyfunc.PythonModel):
    ...
    def predict(self, context, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.debug(pred_A_df)
        pred_B_df = request_api(df, self.predictor_A)
        logger.debug(pred_B_df)
        preds = pd.concat([pred_A_df, pred_B_df])
        logger.info(preds)
        return preds
